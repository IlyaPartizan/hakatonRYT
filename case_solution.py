#функция, опеределеяющая направление движения
#входные параметры:
#prev - координаты начальной точки движения
#cur - координаты конечной точки движения
#выходные параметры:
#'I' или 'II' - маршрут движения(по часовой и против часовой стрелки соответственно
#'stay' - координаты начальной и конечной точки совпадают(поезд не двигается)
#'x' - функция не может дать ответ
...
#функция для рассчета расстояния между точками
#входные параметры:
...
#функция, усредняющая полученные данные
#создает в папке с программой 2 файла:
#res1.csv - усреднение маршрутов, принадлежащих к первому пути
#res2.csv - усреднение маршрутов, принадлежащих ко второму пути
def function_2():
    file = open('bd.csv')
...
#функция, корректирующая координаты точки согласно п.2
#входные параметры:
#dirc - направление движения
#cords - координаты точки
#в папке обязаны быть результаты п.2
#выходные параметры:
...
#класс, служащий для корректирования потока точек
#атрибуты:
#timedelta - ожидание до начала попытки определить расстояние
#dir - направление движения
#trace - история движения
class Corrector:
    def __init__(self):
        self.timedelta = 5
        self.dir = 0
...
#тестирование класса на нескольких первых записях из файла
cor = Corrector()
